...
class Util:
    ...
    def get_primary_display_size(hidpi_scaled=True):
        # Local pin: fractional scaling makes GDK/XWayland report virtual
        # sizes (e.g. 6144x3456 for a 3840x2160 panel at 125%). A file
        # ~/.config/variety/screen_size ("3840x2160") overrides detection
        # for every consumer (wallpaper scaling, clock filter, quotes).
        try:
            with open(os.path.expanduser("~/.config/variety/screen_size")) as _f:
                _w, _h = _f.read().strip().split("x")
